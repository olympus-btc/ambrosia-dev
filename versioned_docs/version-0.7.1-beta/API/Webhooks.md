...
Recibe notificaciones de pago desde el nodo Phoenix.

**Authorization:** No usa JWT — se autentica mediante firma HMAC-SHA256 en el header `X-Phoenix-Signature`.

**Request Headers:**

- `X-Phoenix-Signature` (string, **requerido**): firma HMAC-SHA256 del cuerpo de la petición usando el `webhook-secret` configurado.
...
- La firma se verifica con HMAC-SHA256 usando el secreto `webhook-secret`, leído de la configuración (`phoenix.webhook-secret`) o del archivo `phoenix.conf`. Si el secreto no está configurado, el endpoint rechaza todas las peticiones.
